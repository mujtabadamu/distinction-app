...
const HowtoPractice = ({
  ...
  const { totalSingleQuestion: totalQuestions } = useGetSingleTotalQuestion({
    paperId: selectedExamGroupId,
  });
  // console.log('totalSingleQuestion', totalSingleQuestion);
  console.log('selectedExamGroupId', selectedExamGroupId);
  ...
  // const totalQuestions = totalSingleQuestion ?? 0;
  ...
  useEffect(() => {
    if (isQuizathonInProgress) return; // Prevent override during quizathon
        ...
        <div className="modal-content">
          ...
          >
            ...
            <Box
              ...
              onClick={() => {
                if (!isQuizathonInProgress) setUseDefault('custom');
              }}
            >
              <Box display="flex">
                <RadioButton
