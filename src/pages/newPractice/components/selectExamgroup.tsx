...
const SelectExamGroup = ({
  ...
  useEffect(() => {
    if (papers?.items) {
      if (page === 1) {
        setLocalPapers(papers.items as Paper[]);
      } else {
        setLocalPapers((prev) => [...prev, ...(papers.items as Paper[])]);
      }
    }
  }, [papers, page]);
            ...
            <Grid default="1fr 1fr" sm="1fr">
              ...
              <Box>
                ...
                <CustomSelect
                  ...
                  options={
                    localPapers?.map((v) => ({
                      label: `${capitalizeFirstLetterOFEachWord(v?.name)} (${capitalizeFirstLetterOFEachWord(v.exam.name)})`,
                      value: v?.id,
                    })) || []
                  }
