...
import {DistinctionUploadedUserRegistrationRequest ,DistinctionUploadedUserRegistrationResponse } from "generated/index"

const useValidateStudent = () => {
  const [loadingValidateStudent, setLoadingValidateStudent] = useState<boolean>(false);
  const [isRegisteringUser,setIsRegisteringUser]= useState<boolean>(false);
  const [studentValidated, setStudentValidated] = useState<UploadedStudentView | null>(null);
  const [error, setError] = useState<string | null>(null);

  const getValidateStudent = async (matriculationNumber: string, cb: ()=>void) => {
    ...
    try {
      ...
      if (!data)  return setError("No record found, ensure you entered the Matric/Reg number correctly.");
      cb() 
    } catch (error) {
      setLoadingValidateStudent(false);
      handleError(error);   
